...
`01-왕초보문법.md`에서 `print`, `len`, `range` 같은 몇 개를 이미 써봤다. 이 문서는 그 목록을 넓혀서,
**"파이썬 자체가 기본으로 들고 있는 도구들"**을 체계적으로 정리한다. 여기서도 AI 코드는 잊고,
일상적인 예시로만 설명한다. `03-AI코드문법연계.md`로 넘어가기 전에 이 문서를 먼저 보자.
...
**비유**: 내장 함수는 "손 뻗으면 바로 잡히는 물건"이고, 표준 라이브러리는 "같은 집(파이썬) 안에 있지만 창고(모듈)에서 꺼내와야 하는 물건"이다. 둘 다 "별도로 설치(`pip install`)할 필요는 없다"는 공통점이 있고, 이 점에서 `torch`(설치해야 하는 외부 라이브러리)와 다르다.
...
`01-왕초보문법.md`에서 `for`문으로 직접 돌리며 처리했던 걸, `map`/`filter`는 함수 하나로 표현한다.
...
data\input.txt
```
`os.path.join`은 폴더 이름과 파일 이름을 OS에 맞는 구분자(Windows는 `\`, Mac/Linux는 `/`)로 이어붙여준다. 직접 문자열로 `"data" + "\\" + "input.txt"`라고 쓰면 다른 OS에서 깨지니, 항상 `os.path.join`을 쓰는 게 안전하다. 우리가 `download_data.py`에서 이미 이렇게 썼다.
...
```
`generate.py`에서 `sys.argv[1]`, `sys.argv[2]`로 실행 시 넘긴 값을 읽었던 것이 이 모듈이다.
...
rock
```
`random.seed(42)`를 안 넣으면 실행할 때마다 다른 결과가 나온다. 우리가 `model.py` 학습에서 쓴 `torch.manual_seed(1337)`도 원리가 같다 — "무작위성을 고정해서 같은 실행 결과를 재현 가능하게" 만드는 것.
...
## 4. 참고 — 우리 프로젝트에서 이미 쓴 표준 라이브러리

| 모듈 | 우리 코드에서 쓴 곳 | 용도 |
|---|---|---|
| `os` | `download_data.py`, `train.py` | 경로 조합(`os.path.join`), 폴더 생성(`os.makedirs`) |
| `sys` | `generate.py` | 커맨드라인 인자 읽기(`sys.argv`) |
| `urllib.request` | `download_data.py` | 인터넷에서 파일 다운로드 |

지금까지 안 썼지만 앞으로 자주 볼 만한 모듈로 `math`, `random`, `datetime`, `json`을 같이 다뤘다.
...
| `torch` | **외부** 라이브러리 (표준 아님!) | ✅ `import torch` + `pip install` 필요 | AI 모델 연산 |

`torch`는 표준 라이브러리가 아니라 **우리가 `pip install`로 따로 설치한 외부 라이브러리**라는 점을 다시 한번 구분해두자 (`04-소스구조와라이브러리.md`에서 🟣로 표시했던 것).

---

## 다음 단계

이 문서의 함수/모듈들이 손에 익으면 `03-AI코드문법연계.md`로 넘어가서, 지금까지 배운 것들이 실제
`model.py`/`train.py`/`generate.py` 안에서 어떻게 조합되어 쓰였는지 확인하자.
